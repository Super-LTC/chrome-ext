 ...
 *
 * Everything here is pure (no React, no fetch) so it can be unit-tested via
 * `scripts/test-qm-view-model.ts` (the repo's QM test convention). The
 * components import these helpers; the test imports them too. Keep it pure.
 */
import type {
  QmActionability,
  QmClearUrgency,
  QmMeasureEntry,
  QmMeasureId,
  QmMeasureSummaryCount,
  QmPatientRow,
  QmUpcomingPatientRow,
} from '@core/types/qm-planner.types';
import { qipMeasureSet } from './qip-programs';
...
export function bucketForActionability(a: QmActionability | undefined): MeasureBucket {
  switch (a) {
    case 'team_decision':
      return 'clearable';
 ...
 */
export function isWhatIfClearable(b: MeasureBucket): boolean {
 ...
 *
 * This is why a falls/UTI trigger that's "at-risk" by raw urgency does NOT
 * belong in the at_risk pile — nothing the team does removes it, so it's
 * will_hit. Urgency alone (the old `statusBucketForUrgency`) couldn't make that
 * distinction; actionability is what separates "at risk" from "destined."
 */
export type StatusBucket = 'at_risk' | 'clearable' | 'will_hit';
 ...
 */
export function entryIsActionable(entry: QmMeasureEntry): boolean {
...
export function statusBucketForEntry(entry: QmMeasureEntry): StatusBucket {
...
const STATUS_RANK: Record<StatusBucket, number> = { at_risk: 0, clearable: 1, will_hit: 2 };

/** Sort key: at_risk (0) < clearable (1) < will_hit (2). */
export function statusRank(b: StatusBucket): number {
 ...
 */
export function statusBucketForRow(row: QmPatientRow): StatusBucket {
  let best: StatusBucket = 'will_hit';
 ...
 * no scoring (see memory reference-five-star-qm-scoring).
 */
const FIVE_STAR_MDS: ReadonlySet<QmMeasureId> = new Set<QmMeasureId>([
  'adl_decline',
  'walk_indep_worsened',
  'pressure_ulcer_long',
  'catheter',
...
export function isFiveStarMds(id: QmMeasureId): boolean {
 ...
 * noise like Falls-Any / Behavior) never show under any lens.
 */
export type QmLens = 'five_star' | 'qip' | 'both';

export function measureInLens(
  id: QmMeasureId,
  lens: QmLens,
...
export function crosserForLens(
  crosser: QmUpcomingPatientRow,
  lens: QmLens,
  state: string | null | undefined
): QmUpcomingPatientRow {
  return {
 ...
 */
export function measureRate(c: QmMeasureSummaryCount): { num: number; den: number; rate: number } {
...
export function projectedNum(num: number, clearedCount: number): number {
  return Math.max(0, num - clearedCount);
}

/** Percentage form of a rate; 0 when the denominator is 0. */
export function ratePct(num: number, den: number): number {
  return den > 0 ? (100 * num) / den : 0;
}

/**
 * Whether a day-101 crosser can still land in the CURRENT quarter. A resident
 * only counts toward a measure once they reach long-stay (day-101); if that
 * crossing falls after the quarter locks, they become long-stay in the NEXT
 * quarter and cannot move the current quarter's rate at all. Keeping or
 * preventing such a crosser in the current-quarter what-if is therefore a
 * next-quarter action — the UI must not present them as in-scope here.
 *
 * Both args are ISO `YYYY-MM-DD`, so a lexical compare is a correct date compare.
 */
export function crosserCountsThisQuarter(crossingDate: string, quarterEndIso: string): boolean {
 ...
 * wrong signal: it flags every *target-replaceable* measure (it's `true` on
 * antipsychotic and pressure-ulcer, both `actionType: 'clinical'`) because it
 * assumes "if you code the new MDS clean" — i.e. it presumes the drug was
 * stopped / the wound healed. Seeding on that pre-checks clinical work as if it
 * were done, projecting a fake 0% (a stage-3 ulcer doesn't roll off on a
 * re-ARD). So the seed is restricted to `modification`; clinical / dx_query /
 * time-based / stay-locked triggers are never auto-resolved (the nurse can
 * still toggle them by hand to model "if I get this cleared").
 */
export function reCodeClearableIds(patients: QmPatientRow[], measureId: QmMeasureId): string[] {
  const out: string[] = [];
...
export function urgencyTally(urgencies: QmClearUrgency[]): Record<QmClearUrgency, number> {
  const t: Record<QmClearUrgency, number> = {
    'at-risk': 0,
    urgent: 0,
    routine: 0,
    'stay-locked': 0,
 ...
 */
const MEASURE_META: Partial<Record<QmMeasureId, { short: string; code?: string }>> = {
...
export function shortLabel(id: QmMeasureId, label: string): string {
  return MEASURE_META[id]?.short ?? label.replace(/\s*\(.*$/, '').trim();
}
export function measureCode(id: QmMeasureId): string | undefined {
 ...
 */
export function displayMdsValue(mdsItem: string, value: string): string {
  const v = value.trim();
