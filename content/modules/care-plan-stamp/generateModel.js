...
// chart-quality flags. These helpers marry the two — matched by libraryStdId,
// never touching a row the nurse has edited.
...
/** Index the generate payload's focuses by libraryStdId (rows without one can't be matched → skipped). */
export function polishByStdId(genPayload) {
  const map = new Map();
  for (const f of genPayload?.focuses || []) {
    if (f?.libraryStdId != null && f.libraryStdId !== '') map.set(String(f.libraryStdId), f);
  }
  return map;
}
 ...
 *
 * A row is eligible when its focus carries a libraryStdId with a polished
 * counterpart AND the nurse hasn't touched it (touchedIds — edits, stamps,
...
export function applyPolish(toAddItems, polishMap, touchedIds) {
  ...
  const items = (toAddItems || []).map((item) => {
    ...
    if (stdId == null || stdId === '') return item;
    if (touchedIds?.has?.(item._rowId) || touchedIds?.has?.(item.ruleId)) return item;
    const polished = polishMap.get(String(stdId));
